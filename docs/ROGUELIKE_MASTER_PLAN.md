...
현재 진행도 화면의 개발 미리보기는 저장된 `runDeck`의 현재 슬롯/변형으로 후보를 계산한다. 확인한 카드 보상은 런 덱에 적용·저장된다. 일반 전투와 해금 진행도는 변경하지 않는다.
...
현재 UI 프로토타입에서는 후보 선택 후 `현재 → 교체 후`를 확인하고 런 덱에 적용하거나 취소할 수 있다. 보상 후보는 저장된 런 덱 기준이며, 적용 직전 runId·덱 revision·현재 변형·현재 후보 목록을 재검증한다. 상점/이벤트의 결제·조건 처리는 아직 구현하지 않아 해당 출처의 실제 적용은 허용하지 않는다.
...
`ROGUELIKE_STARTER_LOADOUTS`는 일반 4스타터의 네임드 정규 6장과 네임드 조커 1장을 실제 카드 ID로 고정한다. `roguelikeStarterDeckPlan()`은 이 프리셋을 공통 29슬롯에 덮어써 `variantBySlot`, `namedRegularIds`, `jokerVariantId`, 30개의 `cardBlueprints`를 만든다. 아직 일반 전투 `makeDeck()`에 연결하지는 않으며 v4 초안의 `runDeck`이 이 청사진을 복제해 현재 덱 상태로 보존한다.
...
- 런 초안 v4는 시작 청사진 `deckPlan`과 변경 가능한 `runDeck`을 분리한다. 현재 덱은 카드 30장의 원본 슬롯·변형 ID·순수/기본 와일드 여부와 revision을 저장한다.
- v1~v3 초안은 스타터 청사진으로 최초 덱을 생성한다. v4 로드는 교체를 보존하며 중복 슬롯, 잘못된 변형, 조커 혼입, 잘못된 장수는 거부한다. 손상된 v4 덱을 스타터로 조용히 되돌리지 않는다.
- 보상 미리보기는 현재 런 덱과 현재 해금 풀을 사용한다. 실제 노드 보상 발급/일회성 수령 경제는 아직 없으므로 개발 미리보기에서 여러 차례 교체를 실험할 수 있다. 매 교체 후 revision 증가와 함께 후보를 새로 계산한다.
- 동일 슬롯 교체는 저장 성공 때만 완료로 표시한다. 중복 적용, 삭제/새 런의 오래된 선택, 저장 실패는 변경 없이 거부한다.
- 일반 대전 덱, 클리어/레벨 진행도, 전투 상태는 변경하지 않는다. 다음 단계는 이 런 덱을 별도 실험 전투에 전달하고 전투 카드의 임시 상태가 런 저장 덱으로 역류하지 않게 하는 것이다.
...
- 이 단계는 덱의 실제 전투 호환성을 검증하는 샌드박스다. 노드 보상 1회 수령, 체력/지도 지속, 상점·이벤트 비용은 아직 런 진행으로 구현하지 않는다.
