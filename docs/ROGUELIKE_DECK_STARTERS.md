...
- [x] 다른 카드군 선택을 막는 하드 클래스 제한은 두지 않기
- [x] 동일 랭크+무늬 순수카드 → 네임드 변형 교체 UI/데이터 구조 검토
...
런 초안의 현재 저장 형식은 v5다. 시작 청사진을 복제한 `runDeck`으로 별도 실험전을 시작할 수 있으며, 수동 발급하는 테스트 보상 노드마다 고정 후보 중 1장을 수령하거나 건너뛸 수 있다. 덱 교체와 노드 결과는 함께 저장하고, 실험전 승패는 보상이나 진행도를 바꾸지 않는다. 실제 맵/전투 노드와의 발급 연결은 후속 단계다.
