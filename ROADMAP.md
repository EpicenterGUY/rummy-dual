...
캐릭터/테마/지역/보상까지 포함한 전체 작업안은 `docs/ROGUELIKE_MASTER_PLAN.md`에 보존한다. 기존 스타터 상세는 `docs/ROGUELIKE_DECK_STARTERS.md`를 참고한다. 현재는 공통 시작 → 지역 3연전 → 중간 보스와 보상 프로토타입까지 구현했다. 최종 런 길이, 적/보스 난도, 드롭 확률과 경제 수치는 검증 후 확정한다.
...
- [ ] 중간 보스 이후 추가 지역 선택과 후반 진행 연결 — 현재 구역 완료 뒤 다음 전투는 아직 발급하지 않는다.
- [ ] 지역 명칭과 지역 수 최종 확정
