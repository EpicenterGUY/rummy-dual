...
- [x] 기본 네임드화는 해당 슬롯의 숫자·무늬 분포를 보존한다. 단, M11B의 비대칭 상·하단값 특수 변형은 숫자 보존 원칙의 제한적 예외 후보로 별도 검증한다.
...
트럼프 카드가 시각적으로 상단과 하단에 랭크를 반복 표시하는 구조를 실제 규칙 축으로 확장한다. 일반 카드는 기존처럼 같은 값 `X/X`를 사용하고, 일부 특수 네임드만 `X/Y`처럼 서로 다른 상단값·하단값을 가질 수 있는 방향을 우선 검토한다. 이 단계는 정식 기본 규칙 잠금이 아니라 프로토타입·밸런스 검증 대상이다.

### 기본 방향
- [x] 일반 카드는 `7/7`, `Q/Q`처럼 상단값과 하단값이 동일한 것으로 해석하고 현재 기존 규칙과 완전히 호환
- [x] 일부 특수 네임드에만 `3/7`, `5/K` 같은 비대칭 상·하단값을 허용하는 카드 설계 축 기록
- [x] 카드를 사용할 때 방향을 선택하고, 선택된 방향의 값 하나를 해당 행동의 `현재값`으로 사용하는 방식을 1차안으로 채택
- [x] 반대편 미사용 숫자는 `잔여값`으로 정의해 일부 카드 효과가 참조할 수 있는 공용 설계 후보로 기록
- [x] 무늬와 카드 자체의 슬롯/소유권은 회전해도 변하지 않으며, 비대칭 숫자는 와일드처럼 임의 숫자를 고르는 것이 아니라 카드에 인쇄된 두 값 중 하나만 선택
- [x] 비대칭 숫자는 모든 네임드의 기본 사양이 아니라 희소한 능력/카드 정체성으로 제한해 런·세트 성립 난도를 과도하게 낮추지 않는 방향 유지

### 효과 확장 축
- [x] `정방향 / 역방향`, `현재값 / 잔여값`, `두 값의 차이`, `양쪽 값 일치 여부`를 공용 효과 조건 후보로 기록
- [x] 낮은 현재값을 선택하고 높은 잔여값을 남기는 선택, 또는 높은 현재값을 즉시 조합에 쓰는 선택 사이의 트레이드오프를 핵심 재미 후보로 설정
- [x] 세트에서는 현재값 하나만 기존 숫자 판정에 사용해 동일 숫자 세트 규칙을 유지하는 방향 우선
- [x] 런에서도 우선은 현재값 하나만 기존 연속 숫자 판정에 사용해 현행 런 규칙을 보존하고, 카드 한 장이 동시에 두 숫자를 잇는 `브리지` 판정은 별도 고급 실험으로 분리
- [ ] `상승/하강 연결`, 회전 방향, 잔여값을 참조하는 네임드 효과 샘플 10~20장 설계
- [ ] 숫자 차이가 큰 카드가 단순 상위호환이 되지 않도록 효과 예산/희귀도/패널티 기준 수립

### 규칙·엔진 검증
- [ ] `topRank / bottomRank / activeRank / residualRank`에 해당하는 내부 데이터 구조와 기존 단일 `rank` 호환 방식 설계
- [ ] 카드가 손→조합→회수→버림→재순환될 때 선택 방향이 언제 유지되고 언제 초기화되는지 명문화
- [ ] 공개 조합에 이미 놓인 비대칭 카드를 임의로 다시 회전할 수 있는지 여부 확정 — 기본안은 조합에 들어간 뒤 방향 고정
- [ ] 새 조합 생성·붙이기·다중 붙이기·버스트·체인·런 완주·러미 판정에서 `현재값` 스냅샷 타이밍 확정
- [ ] A/Q/K 경계와 A-2-3 / Q-K-A / K-A-2 런 특수 규칙에서 비대칭 값 회귀 테스트 추가
- [ ] 조커/숫자 변경 효과/복사 효과와 비대칭 값이 중첩될 때 우선순위 명문화
- [ ] CPU가 두 방향의 세트·런 가능성, 즉시 스위치 반환, 다음 턴 가치까지 비교하는 최소 휴리스틱 설계

### UI / 카드 아트 검증
- [ ] 실제 카드 좌상단·우하단 랭크를 서로 다르게 표시하고 180° 회전 선택이 즉시 읽히는 카드 프레임 프로토타입 제작
- [ ] 카드 선택 시 `현재값` 후보 두 개와 예상 세트/런 성립을 미리보기로 표시
- [ ] 이미 조합에 들어간 카드의 선택 방향을 회전 상태 또는 작은 방향 마커로 명확히 고정 표시
...
- [ ] 프로토타입 결과가 좋으면 M0/M11의 정식 카드 규칙으로 승격하고, 좋지 않으면 카드 개별 효과 또는 특정 테마 전용 기믹으로 축소
