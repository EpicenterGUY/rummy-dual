...
- [x] 스타터 청사진 기반 실제 런 덱 저장 및 동일 슬롯 보상 적용 — 초안 v4의 runDeck/revision, 구버전 마이그레이션, 저장 복원, 후보 재검증, 중복·오래된 선택·저장 실패 회귀 포함
- [x] 런 덱 기반 별도 실험 전투 시작과 전투 상태 격리 — 진행도 화면에서 저장된 v4 런 덱 전용 실험전을 시작하며 30개 청사진을 새 전투 카드 객체로 복제. PURE 기본 와일드 조커와 저장된 네임드 변형을 그대로 전투에 전달하되 위치·상태·효과 마커·승패는 런 덱에 역류하지 않고 일반 클리어/레벨/해금 및 M12 표본에서도 제외. 결과 다시 하기는 현재 저장 덱을 새로 복제
- [ ] 실제 노드 보상 발급/1회 수령 및 상점·이벤트 결제/조건 연결
