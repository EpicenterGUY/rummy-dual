...
- [ ] 나머지 3종 런타임: 잔향·불안정·반향 및 표식 카드 범위
...
- [ ] 설계표의 나머지 기존 카드 5종과 상태/동사 후속 구현
...
대개편의 작업 기준은 `effect-system-v3-wave1`이며 main에는 아직 병합하지 않았다. 현재 **기존 카드 64종 개편·53종 유지·5종 구현 대기**, 공용 상태 **15/18종**, SEASON-BLOOM **0/24종 구현(설계 완료)**이다. 7차 묶음의 실제 동작과 검증은 `docs/EFFECT_SYSTEM_3_WAVE7.md`, 전체 설계표는 `docs/CARD_REDESIGN_3.md`를 따른다. 아래 이전 M0~M13 기록의 카드 설명과 충돌하면 현재 개편된 카드에 한해 3.0 명세를 우선한다.
...
- [ ] 나머지 기존 카드 5종과 잔향·불안정·반향·카드 표식 범위
