...
- [ ] 기존 카드 마지막 1종: 보험설계사와 불안정의 공통 수명·정리 처리
...
대개편의 작업 기준은 `effect-system-v3-wave1`이며 main에는 아직 병합하지 않았다. 현재 **기존 카드 68종 개편·53종 유지·1종 구현 대기**, 공용 상태 **16/18종**, SEASON-BLOOM **0/24종 구현(설계 완료)**이다. 8차 묶음의 실제 동작과 검증은 `docs/EFFECT_SYSTEM_3_WAVE8.md`, 전체 설계표는 `docs/CARD_REDESIGN_3.md`를 따른다. 아래 이전 M0~M13 기록의 카드 설명과 충돌하면 현재 개편된 카드에 한해 3.0 명세를 우선한다.
