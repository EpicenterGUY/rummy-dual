...
- [x] 나머지 7종의 처리 계약 확정 — `docs/STATUS_3_CONTRACT.md`, 등록/라이브 구현과 구분
- [x] 기존 122종 전수 효과 설계 — 개편 69종/유지 53종, `design/cards-v3.json`과 자동 생성 `docs/CARD_REDESIGN_3.md`
...
- [x] 6차 상태: 공용 침묵 관문 — 공식 14종. 기록한 제어자 턴 만료·모든 지속/반응 경로·기존 공개 역할 보존
- [x] 7차 상태: 공용 가변 — 공식 15종. 숫자/무늬 선택·성공 시 소비·공개 역할 고정·손패 복귀 시 해제
- [x] 8차 상태: 공용 잔향 — 공식 16종. 실제 수치 기록·절반 복제·복제본 제외·선택 재개 1회
- [x] 9차 상태: 공용 불안정 — 공식 17종. 기록한 제어자 턴 종료 감소·동시 만료·합법 조합 소모/전체 정리
- [ ] 마지막 상태 반향 런타임 및 표식 카드 범위
- [x] 4차 8종 개편 — 누적 48종, 공용 회복/반환 반응과 불굴·과열 보상
- [x] 5차 10종 개편 — 누적 58종, 기본 획득 선택·공개/반환/조합 한정 예약·선택형 귀환
- [x] 6차 3종 개편 — 누적 61종, 잠복자·슈퍼챗·플래시뱅과 침묵 대상 선택
- [x] 7차 3종 개편 — 누적 64종, 황금손·신인 2기생·게임 방송과 가변 대상/사용값 선택
- [x] 8차 4종 개편 — 누적 68종, 재귀 함수·복사기·기념 방송·24시간 내구방송
- [x] 9차 마지막 1종 보험설계사 — 누적 개편 69종/유지 53종, 기존 122종 목표 효과 반영 완료
- [x] 기존 V-SIGNAL/ZERO-SIGHT/POINT-BLANK 60종의 목표 효과 반영 — 개편/유지 카드 모두 설계표와 일치
- [ ] SEASON-BLOOM 24종 신규 구현 및 후속 상태/동사 확장
- [ ] 숫자 변형·소유권/덱 침투 등 후속 고유효과
- [ ] 신규 상태를 반영한 AI 평가와 인간 실전 밸런스 검증

상세 구현 범위·우선순위·초안 해석은 [효과 시스템 3.0](docs/EFFECT_SYSTEM_3.md)과 [3차 구현 계약](docs/EFFECT_SYSTEM_3_WAVE3.md)을 따른다. **기존 122종의 목표 효과 반영은 완료했다. 18종 상태 전체와 SEASON-BLOOM 구현은 아직 완료하지 않았다.**

## 현재 대개편 작업 — 효과 시스템 3.0 / PR #35

대개편의 작업 기준은 `effect-system-v3-wave1`이며 main에는 아직 병합하지 않았다. 현재 **기존 카드 69종 개편·53종 유지·구현 대기 0종**, 공용 상태 **17/18종**, SEASON-BLOOM **0/24종 구현(설계 완료)**이다. 9차 묶음의 실제 동작과 검증은 `docs/EFFECT_SYSTEM_3_WAVE9.md`, 전체 설계표는 `docs/CARD_REDESIGN_3.md`를 따른다. 아래 이전 M0~M13 기록의 카드 설명과 충돌하면 현재 개편된 카드에 한해 3.0 명세를 우선한다.

- [x] 공용 상태/예약, 균열·조합 표식, 런 분할·합병과 기존 카드 40종 개편
- [x] 4차 8종: 수혈·연명·기생충·염상 스트리머·퀵 리로드·응급 후퇴·탄창 비우기·쌍면 조커
- [x] 5차 10종: 사형선고·온기·장물아비·감정사·독점상·역바이럴·100만 구독·ON AIR·콜라보 신청·크로스파이어
- [x] 6차 3종: 잠복자·슈퍼챗·플래시뱅 — 카드 보호/엄폐 교대, 정확한 카드 선택, 공통 침묵 수명
- [x] 7차 3종: 황금손·신인 2기생·게임 방송 — 숫자/무늬 선택, 취소·재검증, 공개 사용값 고정, CPU 연계. 전체 회귀 148/148 통과
- [x] 8차 4종: 재귀 함수·복사기·기념 방송·24시간 내구방송 — 실제 수치 복제, 새 손패 선택, 보존 카드의 다음 배치 잔향. 전체 회귀 151/151 통과
- [x] 9차 보험설계사 1종과 불안정 — 제어자 고정 수명·영역 이탈·동시 만료/정리·치명타 종료. 전체 회귀 154/154 통과
- [ ] 반향·카드 표식 범위
