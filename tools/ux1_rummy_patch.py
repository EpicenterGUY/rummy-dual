...
rep(old,new,"RUMMY completion contract")

rep("function advanceTutorial(){const step=currentTutorialStep();if(!step)return;if(step.id==='intro'){setTutorialStep('board');return}if(step.id==='board'){setTutorialStep('basic');return}}",
"function advanceTutorial(){const step=currentTutorialStep();if(!step)return;if(step.id==='intro'){setTutorialStep('board');return}if(step.id==='board'){setTutorialStep('basic');return}if(step.id==='rummy'&&state.tutorialSegmentDone){showStartScreen();return}}",
"final tutorial navigation")

old="function triggerRummy(w,lastCards,opts={}){let reload=6;const s=sideObj(w),jokerLast=lastCards.some(c=>c.tag==='jokerLast');s.rummyReturnPending=true;s.rummyRecoveryPending=true;if(lastCards.some(c=>c.tag==='rummyPlus1')){reload=7;if(state.switchTarget===w)addShield(w,4)}if(lastCards.some(c=>c.tag==='rummyHeal4')){heal(w,Math.ceil(15/RECOVERY_UNIT));applyStatus(w,'regen',1);if(state.switchPower>=60)addShield(w,4)}if(w==='player')state.rummy++;drawMany(w,reload,false);if(jokerLast&&opts.returned){"
new="function triggerRummy(w,lastCards,opts={}){let reload=6;const s=sideObj(w),jokerLast=lastCards.some(c=>c.tag==='jokerLast');s.rummyReturnPending=true;s.rummyRecoveryPending=true;if(lastCards.some(c=>c.tag==='rummyPlus1')){reload=7;if(state.switchTarget===w)addShield(w,4)}if(lastCards.some(c=>c.tag==='rummyHeal4')){heal(w,Math.ceil(15/RECOVERY_UNIT));applyStatus(w,'regen',1);if(state.switchPower>=60)addShield(w,4)}if(w==='player')state.rummy++;const beforeReloadHand=s.hand.length;drawMany(w,reload,false);if(jokerLast&&opts.returned){"
rep(old,new,"RUMMY before-hand capture")

rep("combatBanner('러미!','rummy',40);log(`${w==='player'?'나':'상대'} 러미! 새 손패 ${reload}장.`,'good');if(w==='player'){state.selected.clear();state.selectionOrder=[];state.boardSelected.clear();state.target=null;endPlayerTurn()}}",
"combatBanner('러미!','rummy',40);log(`${w==='player'?'나':'상대'} 러미! 새 손패 ${reload}장.`,'good');if(w==='player'){tutorialCheckProgress('rummy',{beforeHand:beforeReloadHand,reload,afterHand:s.hand.length});state.selected.clear();state.selectionOrder=[];state.boardSelected.clear();state.target=null;endPlayerTurn()}}",
"RUMMY tutorial hook")
...
rep(old,new,"tutorial start-menu completion state")
...
r=rp.read_text()
old='- [ ] 러미 튜토리얼'
new='- [x] 러미 튜토리얼 — 마지막 손패를 실제로 사용해 `triggerRummy()` → 기본 6장 리필 확인'
if old not in r: raise SystemExit('missing roadmap rummy item')
r=r.replace(old,new,1)
r=r.replace('- [ ] 튜토리얼 완료 상태 저장','- [x] 튜토리얼 완료 상태 저장 — 러미 실습 성공 시 `tutorialCompleted=true` 저장',1)
r=r.replace('- [ ] 튜토리얼 다시 보기','- [x] 튜토리얼 다시 보기 — 완료 후 시작 메뉴를 `다시 보기` 상태로 전환하고 처음부터 재진입',1)
r=r.replace('1. UX1 P1: deterministic 기본 조작 → 세트 → 런 → 붙이기 → 상대 공개 조합 → 스위치 lessons are live; next connect 러미 to the same real-engine tutorial controller.', '1. UX1 P1: 기본 조작 → 세트 → 런 → 붙이기 → 상대 공개 조합 → 스위치 → 러미의 실제 엔진 튜토리얼이 연결됨. 다음은 UX1 P2의 누적 위력/폭발 결과 강조와 자유 연습전.', 1)
rp.write_text(r)
...
ok(script.includes("id:'rummy'") && script.includes("scenario:'rummy'") && script.includes("expectReload:6") && script.includes("completeOn:'rummy'"), 'RUMMY lesson is a deterministic implemented six-card reload scenario');
ok(script.includes("makeTutorialCard('S','K','rummyLast')") && script.includes("p.deck=[makeTutorialCard('C','2','rummyReload')"), 'RUMMY lesson starts with one last card and a fixed six-card personal deck');
ok(script.includes("tutorialCheckProgress('rummy',{beforeHand:beforeReloadHand,reload,afterHand:s.hand.length})"), 'real triggerRummy path reports the resulting hand to the tutorial controller');
ok(script.includes("if(step.expectReload&&context.afterHand!==step.expectReload)return false"), 'tutorial refuses to complete unless the actual reload count matches six');
ok(script.includes("progress.tutorialCompleted=true;saveProgress()"), 'successful final RUMMY lesson persists tutorial completion');
ok(script.includes("tutorialState.textContent=progress.tutorialCompleted?'다시 보기':'시작'"), 'completed tutorial becomes a replayable start-menu entry');
ok(script.includes("if(step.id==='rummy'&&state.tutorialSegmentDone){showStartScreen();return}"), 'final tutorial completion can return cleanly to the main menu');
ok(!html.includes('고정 패 실습은 순차 추가 중입니다.'), 'start screen no longer claims the core fixed-hand tutorial is still being added');
ok(roadmap.includes('- [x] 러미 튜토리얼') && roadmap.includes('- [x] 튜토리얼 완료 상태 저장') && roadmap.includes('- [x] 튜토리얼 다시 보기'), 'UX1 roadmap records RUMMY completion and replay state');
