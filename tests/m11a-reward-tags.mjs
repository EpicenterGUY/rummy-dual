...
ok(source('roguelikeIssueRewardNode').includes('roguelikeRunDeckProfile(draft)')&&!source('roguelikeIssueRewardNode').includes('progress.deckBuild'),'node issuance uses persistent run deck rather than normal battle deck');
ok(source('roguelikeRewardPreviewText').includes('node.picks')&&!source('roguelikeRewardPreviewText').includes('roguelikeRewardCandidates('),'preview displays the issued snapshot without rerolling candidates');
