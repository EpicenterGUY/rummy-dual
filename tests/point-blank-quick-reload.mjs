...
for(const used of [0,1,2]){
 ...
 const before=s.shield,result=g.submitNewMeld('enemy',s.hand.slice(0,3));assert.equal(result,used<2);
 if(used<2){assert.equal(s.shield-before,8);assert.equal(s.newMeldCount,used+1);assert.equal(q.quickReloadConsumedToken,g.state.turnToken)}else{assert.equal(s.shield,before);assert.notEqual(q.quickReloadConsumedToken,g.state.turnToken);assert.equal(g.bestNewMeldForTurn('enemy'),null)}
}
{
 ...
 assert.equal(g.submitNewMeld('enemy',s.hand.slice(0,3)),'full');assert.equal(s.shield,0);assert.notEqual(q.quickReloadConsumedToken,g.state.turnToken);
}
{
 const {g,s,q}=setup();arm(g,q);q.quickReloadConsumedToken=g.state.turnToken;s.hand=[q,g.makeCard('S','J',false,'enemy'),g.makeCard('H','J',false,'enemy'),g.makeCard('C','2',false,'enemy')];assert.equal(g.submitNewMeld('enemy',s.hand.slice(0,3)),true);assert.equal(s.shield,0,'consumed reward is not granted twice');
}
{
 const {g,s,q}=setup();arm(g,q);g.turnStart('enemy');s.hand=[q,g.makeCard('S','J',false,'enemy'),g.makeCard('H','J',false,'enemy'),g.makeCard('C','2',false,'enemy')];assert.equal(g.submitNewMeld('enemy',s.hand.slice(0,3)),true);assert.equal(s.shield,0,'previous turn reward expires');
}
console.log('PASS Quick Reload: clash-only arm, +8 once on either legal create, no third create, no fourth slot, no return exception, expiry');
