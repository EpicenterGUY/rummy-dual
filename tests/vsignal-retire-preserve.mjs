...
ok(script.includes("vGatherAll:['hold','combo','cycle']")&&script.includes("vEndurance:['extend','cycle','status']"),'preservation cards use shared tendencies rather than a dedicated resource');
...
ok(request.includes("allowSkip:optional")&&request.includes("optional=kind==='burst'")&&request.includes("skipLabel:'보존하지 않기'"),'Gather All preservation remains optional; Endurance chooses one card');
ok(request.includes("optional&&sideObj(actor).hand.length===0"),'CPU skips optional BURST preservation when it would replace an empty-hand RUMMY line');
