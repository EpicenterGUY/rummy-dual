...
export function makeGame(sourceHtml=html,seed=1,options={}){
  ...
  const element=()=>({classList:{add(){},remove(){},toggle(){},contains(){return false}},style:{},dataset:{},textContent:'',innerHTML:'',appendChild(){},addEventListener(){},querySelector(){return null},querySelectorAll(){return[]}});
  const document={getElementById:element,querySelector:()=>null,querySelectorAll:()=>[],createElement:element,body:element(),documentElement:element()};
  const ctx=vm.createContext({console,Math:math,document,localStorage:{getItem(){return null},setItem(){}},setTimeout(){},clearTimeout(){},requestAnimationFrame(){},matchMedia:()=>({matches:true}),performance:{now:()=>0},navigator:{},window:{}});
  // Presentation is inert; all rule, status, card-effect, AI and metric functions remain real.
  code+=`\ndeveloperMode=${!!options.developer};\nrender=()=>{};combatBanner=()=>{};flashPile=()=>{};log=()=>{};renderEffectChoiceModal=()=>{};\nObject.assign(globalThis,{state,NAMED,CORE_IDS,progress,${exports.map(n=>`${n}:typeof ${n}==='function'?${n}:undefined`).join(',')}});})();`;
