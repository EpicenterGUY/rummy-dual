...
if marker in s:
    raise SystemExit('P3 tutorial board block already exists')
anchor='</style>'
if s.count(anchor)!=1:
...
.tutorialCoach{position:relative;border:1px solid #56666a!important;border-left:4px solid #6f9690!important;border-radius:10px;background:#263238!important;box-shadow:0 7px 20px #0003,0 1px 0 #ffffff08 inset!important;color:#e8ece8}
...
if r.count(old)!=1:
    raise SystemExit(f'ROADMAP P3 tutorial anchor mismatch: {r.count(old)}')
r=r.replace(old,new,1)
index.write_text(s)
road.write_text(r)
print('P3 tutorial tactical-board finish installed')
