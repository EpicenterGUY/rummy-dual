...
# Mark that on the definition itself so isolated reward tests that load NAMED
# can preserve their established candidate contract without extra globals.
for marker in [
    "'VSHA':{slot:'HA',themeId:'v-signal',",
    "'VSC6':{slot:'C6',themeId:'v-signal',",
    "'ZSH4':{slot:'H4',themeId:'zero-sight',",
    "'ZSS7':{slot:'S7',themeId:'zero-sight',",
    "'PBCA':{slot:'CA',themeId:'point-blank',",
    "'PBS3':{slot:'S3',themeId:'point-blank',",
]:
    text = text.replace(marker, marker + "rewardPool:false,")

# Preserve existing roguelike encounter compositions and reward-ranking
# baselines in this implementation wave. Those integrations get their own
# balance pass later instead of silently changing old run contracts.
...
extra = needle + "\nfor(const id of ['VSHA','VSC6','ZSH4','ZSS7','PBCA','PBS3']){const at=src.indexOf(`'${id}':{slot:`);ok(at>=0&&src.slice(at,at+180).includes('rewardPool:false'),`${id} stays outside random roguelike reward offers for Wave 01A`)}\nok(src.includes(\"if(!def||def.rewardPool===false||String(id).startsWith('J'))return false\"),'roguelike reward candidate filter honors staged named cards');"
