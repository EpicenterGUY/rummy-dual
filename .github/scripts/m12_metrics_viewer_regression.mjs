...
for(const id of ['battleMetricsCount','battleMetricsOverview','battleMetricsList','battleMetricsCopyBtn','battleMetricsClearBtn'])ok(html.includes(`id="${id}"`),`developer panel exposes ${id}`);
...
for(const name of ['battleMetricsAggregate','metricAvg','battleMetricsAggregateText','battleMetricRowText','renderBattleMetricsHistory','copyBattleMetricsHistory','clearBattleMetricsHistory'])ok(script.includes(`function ${name}(`),`viewer helper exists: ${name}`);

{
  const ctx=vm.createContext({console,Math,Number,Array,Object});
  for(const name of ['battleMetricsAggregate','metricAvg','battleMetricsAggregateText','battleMetricRowText'])vm.runInContext(source(name),ctx);
  const rows=[
    {mode:'battle',outcome:'win',turns:10,maxPower:50,bursts:[{}],chains:[{},{}],detonates:[{}],opponentMeldUses:2,multiAttachMax:2,rummys:[{}],maintenance:[{}],intentionalBombAccepts:[{}]},
    {mode:'battle',outcome:'loss',turns:20,maxPower:80,bursts:[{},{}],chains:[{}],detonates:[{},{}],opponentMeldUses:0,multiAttachMax:3,rummys:[],maintenance:[{},{}],intentionalBombAccepts:[]},
    {mode:'practice',outcome:'win',turns:6,maxPower:30,bursts:[],chains:[{}],detonates:[],opponentMeldUses:1,multiAttachMax:1,rummys:[{}],maintenance:[],intentionalBombAccepts:[{},{}]}
  ];
