...
r=r.replace('- [ ] Stabilize first ~50 named cards\n','- [x] Stabilize first ~50 named cards\n',1)
r=r.replace('- [ ] Finish remaining choice/copy/timing audit and per-card regressions before declaring the first ~50 behavior-stable\n','- [x] Finish remaining choice/copy/timing audit and per-card regressions; first ~50 named-card behavior is now locked by executable final-audit coverage\n',1)
anchor='- [x] Final choice pass C: Last Laugh returning-RUMMY now lets the human choose which post-refill card goes to deck bottom, and the RUMMY turn ends only after that mandatory choice resolves\n'
insert=anchor+'- [x] Final lock audit: explicit player-choice cards use shared/resumable selection; deterministic cards keep explicit oldest/reverse/placeholder wording; legacy first-target/auto-discard paths are rejected by regression\n'
if anchor not in r: raise SystemExit('missing M8 final lock anchor')
r=r.replace(anchor,insert,1)
road.write_text(r,encoding='utf-8')

t=Path('tests/m8-final-lock.mjs')
