...
once("function turnEnd(w){const s=sideObj(w);s.creditDebt=false;", "function turnEnd(w){const s=sideObj(w);s.detonateMemory=0;s.creditDebt=false;", 'turnEnd detonate memory expiry')
...
ok(start.includes('if(s.detonateMemory>0){const pi=s.spent.findIndex')&&start.includes('ph.phoenixReturned=true')&&!start.includes('heal(w,3);log(`${ph.name}: 직전 폭발'),'Phoenix may return from spent after DETONATE without receiving its use-triggered heal for free');
